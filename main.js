...
(function() {
    function iniciarSplash() {
        ...
        // 👇 ADICIONE ESTA LINHA - Libera o scroll do body
...
document.getElementById('formAgro').addEventListener('submit', function(e) {
    ...
    if (imp.length > 0) { document.getElementById('manutencaoMaquinas').innerHTML = gerarManutencao(imp); document.getElementById('manutencaoMaquinas').style.display = 'block'; }
    document.getElementById('resultadoAgro').style.display = 'block';
});

function gerarManutencao(imp) {
    let h = '<h3>🔧 Manutenção</h3>';
    if (imp.includes('trator')) h += '<div class="maquina-card"><h4>🚜 Trator</h4><ul><li>10h: Checar óleo</li><li>50h: Graxar</li></ul></div>';
    if (imp.includes('colheitadeira')) h += '<div class="maquina-card"><h4>🌾 Colheitadeira</h4><ul><li>Diária: Limpar</li></ul></div>';
    if (imp.includes('pulverizador')) h += '<div class="maquina-card"><h4>💨 Pulverizador</h4><ul><li>Limpar bicos</li></ul></div>';
    return h;
}

function gerarManejoCultura(cultura) {
    let h = '<h3>📖 Manejo - ' + cultura + '</h3>';
    h += '<div class="cultura-card"><p>🌱 Siga as recomendações da assistência técnica local.</p></div>';
    return h;
}
...
document.getElementById('formLeite').addEventListener('submit', function(e) {
    ...
    // ========== CONTEÚDO INFORMATIVO POR SISTEMA DE CRIAÇÃO ==========
    let textoManejo = '';
    ...
    if (dadosRebanho.sistemaCriacao === 'confinamento') {
        textoManejo = `
            <h4>🌟 Manejo</h4>
            <ul>
                <li><strong>Rotina Rigorosa:</strong> Vacas leiteiras amam rotina. A ordenha (geralmente 2 a 3 vezes ao dia) e o trato devem acontecer rigorosamente nos mesmos horários.</li>
                <li><strong>Manutenção da Cama:</strong> No barracão, as camas de areia ou borracha devem ser limpas e niveladas diariamente. No confinamento, a cama de serragem precisa ser revirada com trator (enxada rotativa) 2 vezes ao dia para se manter seca. Cama úmida causa <strong>mastite</strong> (infecção no úbere).</li>
                <li><strong>Controle Térmico:</strong> Vacas sofrem muito com o calor (estresse térmico), o que faz o leite despencar. Ventiladores e aspersores (borrifadores de água) na pista de trato e na sala de espera da ordenha são obrigatórios.</li>
            </ul>`;
        textoAlimentacao = `
            <h4>🌾 Alimentação</h4>
            <p>A alimentação é fornecida no cocho como <strong>RMT (Ração Total Misturada)</strong>, onde o volumoso e o concentrado são batidos juntos para a vaca não escolher só o mais gostoso.</p>
            <ul>
                <li><strong>Volumoso (Fibra de alta qualidade):</strong> Silagem de milho de excelente qualidade, silagem de sorgo ou feno de alfafa. Representa cerca de 50% a 60% da dieta.</li>
                <li><strong>Concentrado (Energia e Proteína):</strong> Milho moído (fubá), farelo de soja, farelo de algodão ou polpa cítrica.</li>
                <li><strong>Suplementação Vital:</strong> Sal mineral específico para vacas lactantes, bicarbonato de sódio (para evitar a acidez no rúmen/acidose) e altos níveis de cálcio e fósforo.</li>
                <li><strong>Água:</strong> Consumo gigante. Uma vaca confinada de alta produção pode beber mais de <strong>100 a 140 litros de água limpa por dia</strong>.</li>
            </ul>`;
        ...
        textoManejo = `
            <h4>🌟 Manejo</h4>
            <ul>
                <li><strong>Divisão do Dia:</strong> As vacas geralmente passam a noite ou as horas mais frescas do dia no pasto. Nos horários mais quentes, ou logo após a ordenha, vão para o galpão/cocho coberto receber o trato concentrado e descansar na sombra.</li>
                <li><strong>Manejo de Cocho:</strong> O espaço de cocho deve ser suficiente para que todas as vacas comam ao mesmo tempo, evitando que as vacas mais mansas fiquem sem comer.</li>
                <li><strong>Sanidade:</strong> Monitoramento constante de carrapatos e bernes, que são mais comuns por causa do acesso ao pasto.</li>
            </ul>`;
        textoAlimentacao = `
            <h4>🌾 Alimentação</h4>
            <p>O pasto serve como a base de fibra barata, mas o cocho garante a alta produção de leite.</p>
            <ul>
                <li><strong>No Pasto:</strong> Pastagens bem manejadas (como Mombaça, Tifton ou Pioneiro).</li>
                <li><strong>No Cocho:</strong> Fornece-se uma ração concentrada comercial ou batida na fazenda (milho + soja + minerais).</li>
                <li><strong>Regra de ouro:</strong> Geralmente oferece-se <strong>1 kg de ração para cada 2,5 kg ou 3 kg de leite produzidos</strong> acima de uma média base que o pasto sozinho sustentaria.</li>
                <li><strong>Na Seca:</strong> Se o pasto secar, é preciso fornecer um volumoso complementar no cocho (silagem ou cana-de-açúcar corrigida com ureia).</li>
            </ul>`;
        ...
        textoManejo = `
            <h4>🌟 Manejo</h4>
            <ul>
                <li><strong>Pastejo Rotacionado (Obrigatório):</strong> A propriedade deve ser dividida em piquetes (pequenos pastos cercados). As vacas entram em um piquete com o capim na altura certa, comem por 1 ou 2 dias, e mudam para o próximo. O pasto saído precisa de tempo (20 a 30 dias) para crescer de novo.</li>
                <li><strong>Sombra Natural ou Artificial:</strong> O pasto deve ter árvores ou sombrites. Vaca com calor no sol não come e não produz leite.</li>
                <li><strong>Caminhada Curta:</strong> Os piquetes não podem ser muito longe da sala de ordenha, senão a vaca gasta toda a energia andando e cansa os cascos.</li>
            </ul>`;
        textoAlimentacao = `
            <h4>🌾 Alimentação</h4>
            <ul>
                <li><strong>Nas Águas (Chuva):</strong> O capim verde e novo é rico em proteína e energia. A suplementação é feita com <strong>sal mineral específico para leite</strong> no cocho do pasto e uma pequena quantidade de ração na hora da ordenha (geralmente 1 a 2 kg por dia) apenas para "chamar" a vaca e manter a persistência de leite.</li>
                <li><strong>Na Seca (Inverno/Estiagem):</strong> O capim vira "bucha" (perde proteína e fica duro). Aqui o manejo muda: é preciso fornecer <strong>Sal Proteico/Energético</strong> para ajudar a vaca a digerir o capim seco, ou fornecer cana-de-açúcar + ureia/silagem em cochos espalhados pelo pasto.</li>
            </ul>`;
    }

    // Resumo das diferenças de consumo
    let textoResumo = '';
    if (dadosRebanho.sistemaCriacao) {
        textoResumo = `
            <h4>📊 Resumo das Diferenças de Consumo de Água e Comida</h4>
            <ol>
                <li><strong>Confinada:</strong> Come mais comida concentrada e cara, bebe água em bebedouros de alta vazão do lado da cama.</li>
                <li><strong>Semi-confinada:</strong> Equilibra o capim com a ração do cocho. Precisa de água limpa tanto no galpão quanto nos piquetes.</li>
...
document.getElementById('formSuinos').addEventListener('submit', function(e) {
    ...
    if (dadosSuinos.tipoCriacao === 'chiqueiro') {
        textoGuiaSuinos = `
            <h3>📖 Guia - Pequena Produção (Chiqueiro)</h3>
            <div class="maquina-card">
                <h4>🌟 Manejo</h4>
                <p>Instalações simples (chiqueiros ou baias de alvenaria com piso de cimento). É fundamental que tenham uma área coberta para proteção contra sol e chuva, e uma área descoberta (solário) onde os animais possam defecar e urinar. A limpeza deve ser diária, lavando o piso com água para evitar acúmulo de dejetos, mau cheiro e proliferação de moscas. Os leitões recém-nascidos precisam receber ferro injetável nos primeiros dias de vida para evitar anemia.</p>
            </div>
            <div class="maquina-card">
                <h4>🌾 Alimentação</h4>
                <p>Utilizam-se subprodutos da propriedade, como mandioca, abóbora, soro de leite, cana-de-açúcar e milho moído. Restos de comida caseira (lavagem) podem ser fornecidos, desde que sejam totalmente cozidos antes para eliminar microrganismos causadores de doenças. Essa alimentação caseira deve ser complementada com um núcleo mineral e vitamínico misturado ao milho.</p>
            </div>`;
    } else if (dadosSuinos.tipoCriacao === 'chiqueirao') {
        textoGuiaSuinos = `
            <h3>📖 Guia - Grande Produção (Chiqueirão)</h3>
            <div class="maquina-card">
                <h4>🌟 Manejo</h4>
                <p>Altamente tecnificado e dividido em quatro setores físicos isolados: <strong>Maternidade, Creche, Crescimento e Terminação</strong>. As instalações possuem piso ripado (com frestas) para que fezes e urina caiam diretamente em canaletas subterrâneas, mantendo os porcos secos. O ambiente é climatizado com cortinas automáticas ou exaustores para manter a temperatura ideal. Controle sanitário rígido com biossegurança (banho e troca de roupas para entrada na granja).</p>
            </div>
            <div class="maquina-card">
                <h4>🌾 Alimentação</h4>
                <p>Totalmente baseada em rações balanceadas e automatizadas, fornecidas em comedouros automáticos tipo "torta" ou de fluxo constante. As rações são fareladas ou peletizadas, compostas por milho moído e farelo de soja, enriquecidas com aminoácidos sintéticos (lisina e metionina), vitaminas e minerais. Cada fase recebe ração específica: maternidade, pré-inicial, crescimento e terminação.</p>
            </div>`;
    }
...
document.getElementById('formOvelhas').addEventListener('submit', function(e) {
    ...
    if (dadosOvelhas.tipoCriacao === 'pequeno') {
        textoGuiaOvelhas = `
            <h3>📖 Guia - Ovinos (Pequeno Rebanho)</h3>
            <div class="maquina-card">
                <h4>🌟 Manejo</h4>
                <p>Abrigos limpos, arejados e, acima de tudo, secos. Esses animais não toleram umidade no chão, que causa o apodrecimento dos cascos (pododermatite). Realizar casqueamento periodicamente. O maior desafio sanitário são as verminoses; utilizar o método Famacha (monitoramento dos olhos) para aplicar vermífugos apenas nos animais com sinais de anemia, evitando resistência.</p>
            </div>
            <div class="maquina-card">
                <h4>🌾 Alimentação</h4>
                <p>Pastejo livre durante o dia com recolhimento ao abrigo ao final da tarde. Ovelhas preferem gramíneas rentes ao solo. No cocho do abrigo, fornecer sal mineral específico para ovinos. <strong>Atenção:</strong> Ovelhas possuem alta sensibilidade ao cobre, que se acumula no fígado e pode ser fatal — jamais devem consumir sal mineral formulado para bovinos.</p>
            </div>`;
    } else if (dadosOvelhas.tipoCriacao === 'grande') {
        textoGuiaOvelhas = `
            <h3>📖 Guia - Ovinos (Grande Rebanho)</h3>
            <div class="maquina-card">
                <h4>🌟 Manejo</h4>
                <p>Pastejo rotacionado intensivo com piquetes divididos por cercas de tela ou elétrica multifios. Alojamento principal em aprisco suspenso (cerca de 1 metro do chão com ripas espaçadas), permitindo que fezes caiam e urina escoe. Estação de monta controlada para concentrar nascimentos nas épocas de maior oferta de alimento.</p>
            </div>
            <div class="maquina-card">
                <h4>🌾 Alimentação</h4>
                <p>Volumoso dos piquetes adubados combinado com suplementação no cocho. Animais em terminação ou matrizes no terço final de gestação/lactação recebem silagem de milho/sorgo ou feno (Coast-Cross ou Alfafa) + concentrado proteico (milho e soja). Utiliza-se <strong>creep-feeding</strong>: cercados com frestas onde apenas cordeiros entram para comer ração especial de crescimento.</p>
            </div>`;
    }
...
document.getElementById('formCabras').addEventListener('submit', function(e) {
    ...
    if (dadosCabras.tipoCriacao === 'pequeno') {
        textoGuiaCabras = `
            <h3>📖 Guia - Caprinos (Pequeno Rebanho)</h3>
            <div class="maquina-card">
                <h4>🌟 Manejo</h4>
                <p>Abrigos limpos, arejados e secos (não toleram umidade). Casqueamento periódico. Controle de verminoses com método Famacha para evitar resistência aos vermífugos.</p>
            </div>
            <div class="maquina-card">
                <h4>🌾 Alimentação</h4>
                <p>Pastejo livre com recolhimento ao abrigo. Cabras preferem arbustos, folhas largas e ramos altos (hábito de ramoneio). No cocho, fornecer sal mineral específico para caprinos.</p>
            </div>`;
    } else if (dadosCabras.tipoCriacao === 'grande') {
        textoGuiaCabras = `
            <h3>📖 Guia - Caprinos (Grande Rebanho)</h3>
            <div class="maquina-card">
                <h4>🌟 Manejo</h4>
                <p>Pastejo rotacionado intensivo com piquetes e cercas elétricas. Aprisco suspenso para manter ambiente seco. Estação de monta controlada.</p>
            </div>
            <div class="maquina-card">
                <h4>🌾 Alimentação</h4>
                <p>Volumoso dos piquetes + suplementação no cocho com silagem de milho/sorgo ou feno + concentrado proteico (milho e soja). <strong>Creep-feeding</strong> para cabritos com ração especial de crescimento.</p>
            </div>`;
    }
...
document.getElementById('formCavalos').addEventListener('submit', function(e) {
    ...
    if (dadosCavalos.tipoCriacao === 'pequeno') {
        textoGuiaCavalos = `
            <h3>📖 Guia - Equinos (Pequena Criação)</h3>
            <div class="maquina-card">
                <h4>🌟 Manejo</h4>
                <p>Animais soltos no pasto na maior parte do tempo, com abrigo ou baia simples para proteção de tempestades e sol excessivo. Inspeção diária dos cascos para remover pedras e sujeira, e escovação do pelo para identificar ferimentos ou carrapatos.</p>
            </div>
            <div class="maquina-card">
                <h4>🌾 Alimentação</h4>
                <p>Base no capim verde do pasto. Cavalos de trabalho leve recebem ração comercial (1 kg a 2 kg/dia) dividida em duas porções. Sal mineral próprio para equinos em cochos cobertos. Água corrente e limpa (cavalos rejeitam água suja ou parada).</p>
            </div>`;
    } else if (dadosCavalos.tipoCriacao === 'grande') {
        textoGuiaCavalos = `
            <h3>📖 Guia - Equinos (Haras/Grande)</h3>
            <div class="maquina-card">
                <h4>🌟 Manejo</h4>
                <p>Baias individuais amplas (3mx3m ou 4mx4m), ventiladas, com camas grossas de maravalha ou serragem limpas e trocadas diariamente. Rotina fixa de exercícios, treinamentos, banhos e escovação. Casqueamento e ferrageamento a cada 40-50 dias. Controle de parasitas com exames de fezes antes da vermifugação.</p>
            </div>
            <div class="maquina-card">
                <h4>🌾 Alimentação</h4>
                <p>Rigorosamente controlada para evitar cólica e laminite. Base: feno de alta qualidade (Alfafa ou Tifton) fornecido várias vezes ao dia. Ração concentrada balanceada (energia, proteína e óleos) fracionada em 3-4 refeições diárias de no máximo 2 kg cada. Suplementos para articulações, pelagem e musculatura conforme categoria (potros, garanhões, éguas prenhas ou atletas).</p>
            </div>`;
    }
...
document.getElementById('formAves').addEventListener('submit', function(e) {
    ...
    if (dadosAves.tipoProducao === 'ovos') {
        if (dadosAves.tipoInstalacao === 'galinheiro') {
            textoGuiaAves = `
                <h3>📖 Guia - Aves de Postura (Pequena Produção)</h3>
                <div class="maquina-card">
                    <h4>🌟 Manejo</h4>
                    <p>Sistema semiconfinado: galpão/galinheiro coberto para noite, proteção e postura, com acesso a piquetes durante o dia para ciscar. Poleiros para descanso noturno. Ninhos individuais (1 para cada 4-5 galinhas) em locais escuros e silenciosos, forrados com maravalha ou palha seca. Coleta de ovos pelo menos 2 vezes ao dia.</p>
                </div>
                <div class="maquina-card">
                    <h4>🌾 Alimentação</h4>
                    <p>Além do pasto (insetos, minhocas, capim) e milho, é fundamental fornecer ração de postura balanceada farelada com níveis elevados de cálcio para formação da casca. A falta de cálcio faz a galinha retirar o mineral dos próprios ossos, gerando ovos de casca mole e risco de bicagem dos ovos.</p>
                </div>`;
        } else {
            textoGuiaAves = `
                <h3>📖 Guia - Aves de Postura (Grande Produção)</h3>
                <div class="maquina-card">
                    <h4>🌟 Manejo</h4>
                    <p>Sistema de baterias de gaiolas suspensas ou cage-free (aves soltas com ninhos automáticos). Controle ambiental com climatização por pressão positiva/negativa. <strong>Manejo de iluminação:</strong> programa de luz artificial para 15-16 horas diárias, estimulando a glândula pineal a liberar hormônios de ovulação contínua.</p>
                </div>
                <div class="maquina-card">
                    <h4>🌾 Alimentação</h4>
                    <p>Automatizada por calhas ou comedouros tipo corrente. Dividida em fases: inicial (até 6ª semana), crescimento (7ª-15ª), pré-postura e postura (várias fases conforme idade). Ingredientes: milho moído e farelo de soja + aminoácidos sintéticos + vitaminas + calcário calcítico fino e grosso para cálcio disponível durante a noite (formação da casca).</p>
                </div>`;
        }
    } else if (dadosAves.tipoProducao === 'carne') {
        if (dadosAves.tipoInstalacao === 'galinheiro') {
            textoGuiaAves = `
                <h3>📖 Guia - Frango de Corte (Pequena Produção/Caipira)</h3>
                <div class="maquina-card">
                    <h4>🌟 Manejo</h4>
                    <p>Linhagens de crescimento lento adaptadas à vida ao ar livre. Pintinhos passam 3-4 semanas em galpão fechado com aquecimento artificial (lâmpadas/campânulas) sobre cama de maravalha. Após a cria, acesso a piquetes durante o dia. Ciclo total: 70 a 90 dias até o abate (carne mais firme e coloração acentuada).</p>
                </div>
                <div class="maquina-card">
                    <h4>🌾 Alimentação</h4>
                    <p>Ração comercial de crescimento nas primeiras semanas. Após saída para o pasto, introdução de milho moído/quebrado. Consumo de capim e insetos complementa a nutrição. O caroteno das pastagens verdes e do milho dá a tonalidade amarela intensa à pele e gordura do frango caipira.</p>
                </div>`;
        } else {
            textoGuiaAves = `
                <h3>📖 Guia - Frango de Corte (Grande Produção/Industrial)</h3>
                <div class="maquina-card">
                    <h4>🌟 Manejo</h4>
                    <p>Linhagens de altíssimo desempenho genético em galpões Blue House ou Dark House (luz natural bloqueada para controle absoluto de luz/escuridão). Controle de temperatura com exaustores e painéis evaporativos. <strong>Vazio sanitário</strong> de 10-14 dias entre lotes para quebrar ciclo de patógenos. Frango atinge 2,5-3 kg em apenas 42 dias.</p>
                </div>
                <div class="maquina-card">
                    <h4>🌾 Alimentação</h4>
                    <p>Sistemas automáticos com rações peletizadas (evita separação de ingredientes). Dieta de alta densidade energética/proteica dividida em: pré-inicial (aparelho digestivo), inicial (estrutura óssea), crescimento/engorda (massa muscular), e ração de retirada/pré-abate (sem medicamentos, para segurança do consumidor).</p>
                </div>`;
        }
    } else if (dadosAves.tipoProducao === 'ambos') {
        textoGuiaAves = ''; // não acrescentar nenhum texto
    }
